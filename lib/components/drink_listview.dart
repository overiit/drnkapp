...
class _DrinkListViewState extends State<DrinkListView> {
  ...
  Widget buildDrink(Drink drink, {bool isLast = false}) {
    IconData icon = Icons.liquor_rounded;
    if (drink.type == DrinkType.beer) {
      icon = Icons.sports_bar_rounded;
    } else if (drink.type == DrinkType.wine) {
      icon = Icons.wine_bar_rounded;
    } else if (drink.type == DrinkType.spirit) {
      icon = Icons.local_bar_rounded;
    } else if (drink.type == DrinkType.other) {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: widget.drinks.map((drink) => buildDrink(drink, isLast: widget.drinks.indexOf(drink) == widget.drinks.length - 1)).toList(),
    );
  }
}
